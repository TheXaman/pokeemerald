...
void LoadTilesetPalette(struct Tileset const *tileset, u16 destOffset, u16 size)
{
    ...
    if (tileset)
    {
        ...
        {
            gPaletteOverrides[0] = tileset->paletteOverrides; //day_night 
            LoadPalette(&black, destOffset, 2);
            LoadPaletteDayNight(((u16*)tileset->palettes) + 1, destOffset + 1, size - 2); //day_night 
            nullsub_3(destOffset + 1, (size - 2) >> 1);
        }
        else if (tileset->isSecondary == TRUE)
        {
            gPaletteOverrides[1] = tileset->paletteOverrides; //day_night 
            LoadPaletteDayNight(((u16*)tileset->palettes) + (NUM_PALS_IN_PRIMARY * 16), destOffset, size); //day_night 
            nullsub_3(destOffset, size >> 1);
        }
        else
        {
            gPaletteOverrides[2] = tileset->paletteOverrides; //day_night 
            LoadCompressedPaletteDayNight((u32*)tileset->palettes, destOffset, size); //day_night 
            nullsub_3(destOffset, size >> 1);
        }
    }
}
